    ...
    <div class="modal fade flip" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('order.statustodispatched') }}" onsubmit="return validatedocket(event);"  autocomplete="off"
                    enctype="multipart/form-data" id="courierform">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="docketNo" id="docketno"
                                placeholder="Enter Docket No" maxlength="100" autocomplete="off"  required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <div class="hstack gap-2 justify-content-end">
                            <button type="submit" class="btn btn-primary btn-user float-right mx-2" id="add-btn">Submit</button>
...
@section('scripts')
    ...
    if (docketno.length === 0) {
        $("#courierform")[0].submit(); // Submit if docket number is empty
        return true;
    }

    var url = "{{ route('order.validatedocketno') }}";

    // Use AJAX call and handle it with a promise
    $.ajax({
        url: url,
        type: 'GET',
        data: {
        ...
        if (data == 1) {
            $("#docketno").val('');
            alert('Docket Number Already Exists.');
            return false; // Prevent form submission
        } else {
            $("#courierform")[0].submit(); // Manually submit the form if valid
        }
    }).fail(function() {
