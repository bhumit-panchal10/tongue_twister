...
class RazorpayController extends Controller
{
    ...
    {
        $Order = Order::where("tt_order_id", $request->orderId)->where(['iStatus' => 1, 'isDelete' => 0])->first();
        ...
        Payment::where('oid', $orderId)->update($data);

        $payment=Payment::where('oid',$orderId)->first();
        ...
        if ($generated_signature == $razorpay_signature) 
        {
            ...
            $updateData = Payment::where('oid', $orderId)->update([
                'status' => 'Success',
                'iPaymentType' => 1,
                'iSubscriptionYear' => 1,
             ...
             if ($updateData) 
             {
                ...
                Order::where("order_id", $payment->oid)->update($updateProfileData);
             }
            ...
            foreach ($cartItems as $cartItem) {
                $html .= '<tr>
                    <td>' . $i . '</td>
                    <td>' . $cartItem['name'] . '</td>
                    <td><img width="48" height="48" src=http://tongue-twister.in/Product/Thumbnail/' . $cartItem['attributes']['image'] . '></td>
